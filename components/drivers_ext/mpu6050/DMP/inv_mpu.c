...
#define MOTION_DRIVER_TARGET_MSP430		//定义驱动部分,采用MSP430的驱动(移植到STM32F0)
...
uint8_t run_self_test(void)
{
	...
	if (result == 0x3) 
	{
		...
		*/
		...
		accel_sens = 0;//使上电初始化的角度参数和位置没有关系，
		accel[0] *= accel_sens;
		accel[1] *= accel_sens;
		accel[2] *= accel_sens;
...
uint8_t mpu_dmp_init(void)
{
	...
	{	     
		...
		if(res)return 2; //2029
		res=mpu_set_sample_rate(DEFAULT_MPU_HZ);	//设置采样率
		if(res)return 3; //2028
		res=dmp_load_motion_driver_firmware();		//加载dmp固件
		if(res)return 4; //1219
		res=dmp_set_orientation(inv_orientation_matrix_to_scalar(gyro_orientation));//设置陀螺仪方向
		if(res)return 5; //1215
		res=dmp_enable_feature(DMP_FEATURE_6X_LP_QUAT|DMP_FEATURE_TAP|	//设置dmp功能
		    DMP_FEATURE_ANDROID_ORIENT|DMP_FEATURE_SEND_RAW_ACCEL|DMP_FEATURE_SEND_CAL_GYRO|
		    DMP_FEATURE_GYRO_CAL);
		if(res)return 6; //1041
		res=dmp_set_fifo_rate(DEFAULT_MPU_HZ);	//设置DMP输出速率(最大不超过200Hz)
		if(res)return 7; //1040      
		//res=run_self_test();		//自检
		//if(res)return 8; //52      
		//注释掉自检程序就是只能在绝对静止(相对于地球是静止的)地方工作,注释掉原因：这个自检初始化时间较慢，影响客户体验；
		//设备并不会出现在相对运动中使用
...
//pitch:俯仰角 精度:0.1°   范围:-90.0° <---> +90.0°//将pitch角度修改为-180 --- +180度之间
...
//		*yaw   = atan2(2*(q1*q2 + q0*q3),q0*q0+q1*q1-q2*q2-q3*q3) * 57.3;	//yaw
        ...
        //pitch 值得范围改到-180 --- +180之间
        *roll = atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2* q2 + 1)* 57.3; // roll
        if(fabs(*roll ) < 90.0) 
            *pitch = asin(-2 * q1 * q3 + 2 * q0* q2)* 57.3;
        else  
            *pitch = (fabs(q0* q2 - q1 * q3)/(q0* q2 - q1 * q3)) *(180.0 - fabs(asin(-2 * q1 * q3 + 2 * q0* q2)* 57.3));
        *yaw   = atan2(2*(q1*q2 + q0*q3),q0*q0+q1*q1-q2*q2-q3*q3) * 57.3; //yaw
	}else return 2;
	return 0;
}
