...
#define APP_ADV_TIMEOUT_IN_SECONDS       18000          //广播时间是300分钟                /**< The advertising timeout in units of seconds. */
...
APP_TIMER_DEF(m_battery_notify_timer_id); 
APP_TIMER_DEF(m_usual_timer_id);

volatile static uint8_t timer_power_on_flag;
volatile static uint16_t timer_power_on_count;
...
{
	UNUSED_PARAMETER(p_context);
	//power off 判断,超过500ms就进行关机处理
	if(timer_power_on_flag == 1)
	{
		timer_power_on_count++;
		if(timer_power_on_count >= 1)
		{
			timer_power_on_flag = 2;
			timer_power_on_count = 0;
		}
	}
	else if(timer_power_on_flag == 2)
	{
		if(nrf_gpio_pin_read(POWER_IO_IN) != 1) 
		{
			timer_power_on_flag = 3;
		}
	}
	else if(timer_power_on_flag == 3)
	{
		if(nrf_gpio_pin_read(POWER_IO_IN) == 1)
		{
			timer_power_on_count++;
		...
		if(timer_power_on_count >= 500)
		{
			nrf_gpio_pin_clear(POWER_IO_OUT);
			timer_power_on_count = 0;
		}
	}
}
...
static void timers_init(void)
{
    ...
    err_code = app_timer_create(&m_battery_notify_timer_id,
                                ...
                                battery_notify_timeout_handler);
    APP_ERROR_CHECK(err_code);
	
    err_code = app_timer_create(&m_usual_timer_id,
                                APP_TIMER_MODE_REPEATED,
                                usual_timeout_handler);
    APP_ERROR_CHECK(err_code);
}
...
void bsp_event_handler(bsp_event_t event)
{
    ...
    switch (event)
    {
        case BSP_EVENT_SLEEP:
			SEGGER_RTT_printf(0,"BSP_EVENT_SLEEP\r\n");
            sleep_mode_enter();
            break;

        case BSP_EVENT_DISCONNECT:
			SEGGER_RTT_printf(0,"BSP_EVENT_DISCONNECT\r\n");
        ...
        case BSP_EVENT_WHITELIST_OFF:
			SEGGER_RTT_printf(0,"BSP_EVENT_WHITELIST_OFF\r\n");
...
static void advertising_init(void)
{
    ...
    APP_ERROR_CHECK(err_code);
	SEGGER_RTT_printf(0,"advertising_init_OK\r\n");
}
...
int main(void)
{
	...
	{
		if((nrf_gpio_pin_read(POWER_IO_IN) == 1)&&(timer_power_on_flag == 0))
		{
			timer_power_on_flag = 1;
		}
		else
		{
			if(timer_power_on_flag == 2)
			{
					nrf_gpio_pin_set(POWER_IO_OUT);
					break;
			}
		}
	}
